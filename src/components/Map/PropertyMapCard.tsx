...
import { MapPin, Eye, Edit, Trash2, Phone, Mail, Store, Heart } from 'lucide-react';
...
export const PropertyMapCard: React.FC<PropertyMapCardProps> = ({
  ...
  // Show verified badge for owner/publisher
  const { verified: isVerified } = useVerification(property.user_id);

  // استخدام hook المفضلة
  const { isFavorite, toggleFavorite, isLoading: isFavoriteLoading } = useFavorite(property.id);
