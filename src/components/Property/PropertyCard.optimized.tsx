...
/**
 * PropertyCard Component - Optimized Version
 * 
 * تحسينات الأداء:
 * 1. استخدام React.memo لمنع Re-renders غير الضرورية
 * 2. استخدام useMemo للقيم المحسوبة
 * 3. استخدام useCallback للدوال
 * 4. تحسين شروط المقارنة في memo
 */
export const PropertyCard = memo(({
    ...
    // استخدام useMemo للقيم المحسوبة التي لا تتغير كثيراً
    const resolvedMarket = useMemo(
        () => resolveMarketValue(property.market ?? property.location ?? property.address ?? property.marketLabel ?? undefined),
        [property.market, property.location, property.address, property.marketLabel]
    );

    const marketLabel = useMemo(
        () => property.marketLabel ?? (resolvedMarket ? getMarketLabel(resolvedMarket) : null),
        [property.marketLabel, resolvedMarket]
    );
    ...
    const isHidden = useMemo(
        () => optimisticHidden !== undefined ? optimisticHidden : property.is_hidden,
        [optimisticHidden, property.is_hidden]
    );
    ...
    const optimizedImageUrl = useMemo(
        () => property.images?.[0] ? getOptimizedImageUrl(property.images[0], isMobile ? 'small' : 'medium') : null,
        [property.images, isMobile]
    );
    ...
    // استخدام useCallback للدوال لمنع إعادة إنشائها في كل render
        ...
        <Card className="overflow-hidden hover-lift shadow-md group relative border-2 border-gray-100">
            ...
            {user && (
                <button
                    ...
                    disabled={isToggling}
                    className={`absolute top-3 ${showCheckbox ? 'left-16' : 'left-3'} z-10 p-2 rounded-full bg-white/90 backdrop-blur-sm shadow-md transition-all hover:scale-110 ${isFav
                            ? 'text-red-500'
                            : 'text-gray-400 hover:text-red-500'
                        }`}
                >
                    <Heart
                        className={`w-4 h-4 transition-all ${isFav ? 'fill-current' : ''
                            }`}
                    />
                </button>
            )}
            ...
            >
                ...
                <div className="relative h-48 overflow-hidden">
                    {optimizedImageUrl ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 text-gray-500 relative">
                            ...
                            <div className="mt-3 text-center">
                                <span className="text-sm font-medium text-gray-600">{propertyTypeLabel}</span>
                    ...
                    <Badge
                        variant="default"
                        className={`absolute top-3 right-3 backdrop-blur-sm font-bold z-10 ${property.listing_type === "sale"
    ...
    // دالة مقارنة مخصصة لـ React.memo
    // ترجع true إذا كانت الـ props متساوية (لا حاجة لإعادة التصيير)
    // ترجع false إذا كانت الـ props مختلفة (يجب إعادة التصيير)

    return (
        prevProps.property.id === nextProps.property.id &&
        prevProps.property.title === nextProps.property.title &&
        prevProps.property.price === nextProps.property.price &&
        prevProps.property.is_published === nextProps.property.is_published &&
        prevProps.property.is_hidden === nextProps.property.is_hidden &&
        ...
        prevProps.onTogglePublication === nextProps.onTogglePublication &&
        prevProps.onSelectionChange === nextProps.onSelectionChange
    );
});

// إضافة displayName للمكون لتسهيل التصحيح
