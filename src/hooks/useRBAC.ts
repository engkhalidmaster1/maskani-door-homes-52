...
export const useRBAC = (options: UseRBACOptions = {}): UseRBACReturn => {
  ...
  const highestRole = useMemo(() => {
    try {
      return rbacManager.getUserHighestRole(userId);
    } catch (err) {
  ...
  const isAdmin = useMemo(() => {
    return rbacManager.isAdmin(userId);
  }, [userId]);

  const isSuperAdmin = useMemo(() => {
    return rbacManager.isSuperAdmin(userId);
  }, [userId]);

  // Memoized navigation
  const allowedNavigation = useMemo(() => {
    try {
      return rbacManager.getNavigationForUser(userId);
    } catch (err) {
